...
$result = $conn->query("SELECT `i`.`item_id`, `i`.`name`, `il`.`item_line_id`, 
                `il`.`price`, `il`.`qty`, `il`.`old_stock`,
                `s`.`stock_transaction_id`, `s`.`transaction_timestamp`, 
                `s`.`type`, `bi`.`qty` AS 'stock_qty'
