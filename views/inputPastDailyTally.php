...
<div style="width:40%; margin-left:1%;">
  Branch
  <input id="branch" class="form-control" disabled>
</div>
<div class="form-group" style="width:30%; margin-left:1%; margin-top:2%;">
  <label class=" form-control-label">Tally Date <i class="fa fa-calendar"></i></label>
  <input id="date" class="form-control" value="<?php echo $_GET['date'];?>" disabled>
</div>
                ...
                <div id="step-1" class="">
                  <div class="card">
                    <div class="card-header">
                      ...
                      <input id="date" name="date" value="<?php echo $_GET['date'];?>" hidden>
                      <input id="branch_id" name="branch_id" value="<?php echo $_GET['branch_id'];?>" hidden>
                    </div>
                    <div class="card-body">
...
$(document).ready(function(){
  ...
  $('#dailyTally').submit(function(e) {
    ...
    if (parseFloat($('#moneyTotalValue').html()) < parseFloat($('#expectedEarnings').html())) {
      ...
      $.ajax({
        type: "POST",
        url: "../queries/insert/addDailyTallyPastDate.php",
        data: $(this).serialize(),
      }).done(function( data ) {
        if (data == 1) {
...
function PopulateTallyTable() {
  var exists = false;
  var url = new URL(window.location.href);
  var id = url.searchParams.get("branch_id");
